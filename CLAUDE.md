...
| 项目 | 框架 | 测试数量 | 位置 |
|------|------|----------|------|
| relay-server | `cargo test` / `tokio::test` | 94 | `#[cfg(test)]` 内联在源文件中 |
...
| 项目 | 构建命令 | 运行镜像 |
|------|----------|----------|
| relay-server | `docker build -t remote-claude/relay-server -f relay-server/Dockerfile .` | `debian:bookworm-slim` |
...
构建镜像时会**自动编译 desktop-client 二进制文件**并放入下载目录，无需手动放置。

```bash
# 构建镜像（注意 context 为项目根目录）
docker build -t remote-claude/relay-server -f relay-server/Dockerfile .
